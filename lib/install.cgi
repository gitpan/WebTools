  ...
  if(sql_errno($dbh))
    {
     ...
     print "SQL error message: ".sql_errmsg($dbh)."<BR><BR>";
     print "Hint: Check write/read permissions of your default db directory($db_path)!<BR>More about permissions read in INSTALL.html</B>".$f_e;
     exit;
    }
 print << "END_OF_HTML";
...
END_TERM
  eval $code;
  if(($@ ne '') or ($sqlq eq '') or (sql_errno($dbh) ne '')) { print $f_b."<B>Sorry can't create table!<BR> Maybe your CREATE TABLE syntax is not correct or not supported from Flat DB ???</B>".$f_e; exit; }
  print << "END_OF_HTML";
 <B>$f_b
 <H3><U>WebTools Configure</U></H3>
 <table width="100%"><TR><TD width="100%" bgcolor="#D0D080">$f_b
